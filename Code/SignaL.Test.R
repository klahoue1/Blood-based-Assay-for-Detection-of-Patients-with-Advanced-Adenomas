...
FullMetaData = read.csv(file.path(dir,'FullMetaTesting.csv'))
TestingNames=FullMetaData$douville_name
  ...
  
{
  ...
  Adenom = readRDS(file.path(dir,paste0('AmpAdenomFullAmp', nchr, '.rds')))
  #Adenom=rbind(Adenom,readRDS(paste0('AmpExactFullAmp',nchr,'.rds')))
  
  
  #The Cancers
  Cancers = FullMetaData$douville_name[FullMetaData$cancer == TRUE]
  Cancers = intersect(Cancers[!is.na(Cancers)], TestingNames)
  
  #The Normals
  Normals = FullMetaData$douville_name[FullMetaData$cancer == FALSE]
  Normals = intersect(Normals[!is.na(Normals)], TestingNames)
  
  #Split into cancers and normals
  Amp1 =unique(rbind(filter(AmpPrim, douville_name %in% Cancers),Adenom))
  ...
  saveRDS(WP, file.path(dir,paste0('NonNegativeMatrixP', nchr)))
  saveRDS(WQ, file.path(dir,paste0('NonNegativeMatrixQ', nchr)))
  
  S0 = filter(S0,!is.na(apply(S0[, 2:ncol(S0)], 1, sum)))
  S1 = filter(S1,!is.na(apply(S1[, 2:ncol(S1)], 1, sum)))
  write.csv(S0,
            file.path(dir,paste0('NMFScoresNormalsTest', nchr)),
            row.names = FALSE)
  write.csv(S1,
            file.path(dir,paste0('NMFScoresCancersTest', nchr)),
            row.names = FALSE)
...
saveRDS(BV_Normals, file.path(dir,'bvNormalsTest'))
saveRDS(BV_Cancers, file.path(dir,'bvCancersTest'))
